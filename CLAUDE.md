...
**Current status:** Steps 1–3 delivered & verified (`workbook/US_MA_Tax_Scheduling_Tracker_v2_2026-07-19.xlsx`). Step 2 completed by applying the team's 7 Review decisions (only real change: Patch MIF_1 → Active, 9→10 active / 72→80 h/wk). Step 3 added the **Templates tab** (8 deal archetypes → hrs/wk by level + a diligence/tail phase shape), a **Deals!Effort archetype** dropdown, an archetype-driven assignment rate (override → archetype → flat fallback), and **time-phased Capacity** (2-SUMIFS segment design, §7). Verified: 0 errors across 27,002 cells; no-archetype invariant holds (numbers identical to Step 2); synthetic archetype+dates test hand-matched (SA rate 10→14, front 19.6 / tail 8.4 / past-end 0). Still open for the team: which-Kyle on 4 projects, and the real per-level/per-archetype hour numbers (all placeholders, flagged). **Next: Step 4 (NetSuite actuals CSV import + variance).** The generator is still authoritative (Step 2's human input was 7 diff-verified cells, encoded in the script); it becomes bootstrap-only once per-deal operational data is entered at scale — see rule 2.
