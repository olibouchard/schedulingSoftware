...
**Current status: Steps 1–7 delivered & verified** (`workbook/US_MA_Tax_Scheduling_Tracker_v2_2026-07-19.xlsx`, 12 tabs; 0 errors across 49,337 cells at last full verify). **Feedback Round 1 (PLAN.md §5A, Steps 8–11) is planned and its 5 decisions are RECORDED (July 21)**: visible tabs = Dashboard·Deals·Assignments (rest hidden, incl. Roster); **fees per workstream drive hour allocation** — `hours(level,ws) = fee(ws) × split%(ws,level) ÷ rate(level)`, per-assignment = level hrs/wk ÷ active assignees at that level (see §5A "Computed fee→hours design" incl. the four ★ interpretation points); timeline flags as proposed off `Settings!B19`; workstreams replace archetypes (legacy hidden); live copy has NO manual edits → generator still authoritative. **Awaited from the team: the rate card by level + the split-% matrix** (yellow placeholders until then). **Dev is gated on the user's explicit "go" — do NOT start Step 8 without it.** Step 8 (Opus) still opens with the live-copy diff; first real data entry flips the generator to bootstrap-only (rule 2). Older open items: which-Kyle on 4 projects; NetSuite go-live; "missing level"/"stale" definitions.
