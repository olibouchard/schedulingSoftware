...
**Current status: Steps 1–8 delivered & verified** (`workbook/US_MA_Tax_Scheduling_Tracker_v2_2026-07-19.xlsx`, 12 tabs; 0 errors across 49,485 cells at last full verify). Feedback Round 1 (PLAN.md §5A) decisions recorded July 21. **Step 8 done:** retired the empty legacy `Deals!K–P` attributes in place (relabelled + hidden, not deleted) and appended Justine's workstream taxonomy verbatim at **Deals AH–AX** (input dropdowns; lists on Settings M–Q); reworked **Templates** into the fee-allocation parameter tab (rate card $/hr, split-% matrix by workstream group with =100% checksums, default durations — all yellow placeholders), archetype table kept legacy in place. Invariant verified: with no fees/workstreams entered every live number is unchanged (Step 8 added only data columns + parameter tables, no formula edits). **Next session: Step 9 (fee→hours engine, Opus)** — adds per-workstream-group fee columns on Deals and wires `hours(level)=fee×split%÷rate` (spread over kick-off→delivery or default duration; per-assignment = level hrs/wk ÷ active assignees at that level) through the `Assignments!I` choke point; **awaits the team's real rate card + split-% matrix** (placeholders until then). Then Step 10 (timeline flags, Sonnet), Step 11 (Dashboard + tab diet, Opus). Older open items: which-Kyle on 4 projects; NetSuite go-live; "missing level"/"stale" definitions.
