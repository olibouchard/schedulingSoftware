...
**Current status:** Steps 1–6 delivered & verified (`workbook/US_MA_Tax_Scheduling_Tracker_v2_2026-07-19.xlsx`). Steps 2–5: applied the team's 7 Review decisions (Patch MIF_1 → Active); **Templates** (archetypes → hrs/wk by level + phase shape) + time-phased Capacity; **Actuals** (NetSuite CSV import + matching) + **Variance** (calibration); **Check-in** (formula-only weekly agenda: over/under-allocation + deals needing attention). Step 6 hardened it: **all 11 sheets protected** (formulas locked, input cells + spare rows unlocked, no password; insert/delete/sort disabled to protect the row-for-row mirroring), a **data-health panel** on the Review tab (unknown-person/code, duplicate codes, unstaffed deals — integrity checks green on the live file, proven to fire via injected bad data), Check-in **print setup**, and Guide polish. Verified: 0 errors across 35,295 cells; Steps 1–5 numbers unchanged. Caught a real bug: `COUNTIF(range,"?")` treats `?` as a wildcard → escape as `"~?"` (§7). Still open for the team: which-Kyle on 4 projects; real per-level/archetype hour numbers; NetSuite go-live; confirm "missing level"/"stale" definitions. **Next: Step 7 (scenario toggle, Opus — OPTIONAL, build only on request).** The generator is still authoritative (only encodable human input so far); bootstrap-only once real actuals/dates/probabilities are entered at scale — see rule 2.
