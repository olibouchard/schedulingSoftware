...
**Current status: Steps 1–9 delivered & verified** (`workbook/US_MA_Tax_Scheduling_Tracker_v2_2026-07-19.xlsx`, 12 tabs; 0 errors across 57,771 cells at last full verify). Feedback Round 1 (PLAN.md §5A) decisions recorded July 21. **Step 9 done — fee→hours engine:** `Deals` gained per-workstream-group **fee inputs** (AY–BC) + hidden calc helpers — fee-total gate (BD), per-group durations (BE–BI), and **fee hrs/wk by level** (BJ–BP = `Σ fee×split%÷rate÷duration`). Wired through the **`Assignments!I` choke point**: Override → **fee-driven** (`AP` = level hrs/wk ÷ active assignees at that level; `AO` = deal fee total, the gate) → archetype → level default. Hours spread flat over kick-off→delivery (seg1 window now honours a fee-priced deal's dates; Scenario mirror got the same). **Still placeholders:** the rate card + split-% matrix on Templates — the engine computes real hours the instant the team fills them; fees are entered per deal over time. **Invariant verified:** with no fees entered every live number is byte-identical to Step 8 (gated on fee>0); two synthetic scratch tests hand-checked the fee math, even split, unstaffed-level behaviour, date-windowing, and conservation. **Deferred to Step 11:** a visible unstaffed-funded-level flag (caveat documented in Guide + Review for now). **Next: Step 10 (timeline flags, Sonnet)**, then Step 11 (Dashboard + tab diet, Opus). Older open items: which-Kyle on 4 projects; NetSuite go-live; "missing level"/"stale" definitions.
