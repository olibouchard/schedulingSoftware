...
**Current status:** Steps 1–5 delivered & verified (`workbook/US_MA_Tax_Scheduling_Tracker_v2_2026-07-19.xlsx`). Step 2 applied the team's 7 Review decisions (only real change: Patch MIF_1 → Active). Step 3 added the **Templates tab** (archetypes → hrs/wk by level + phase shape) and time-phased Capacity. Step 4 added **Actuals** (NetSuite CSV import + matching) and **Variance** (estimate-vs-actual calibration). Step 5 added the **Check-in tab**: a formula-only weekly-meeting agenda — who's over/under-allocated in the next 4 weeks, and which Active/Proposal deals are unstaffed, missing an archetype-expected level, still at the default 50% probability, dateless, or stale (needs new `Deals!Z` "Added on" + `Settings!B19/B20`). Verified: 0 errors across 35,269 cells; both agenda blocks independently recomputed from raw data with **0 mismatches across 23 people and 306 deals**; the two paths inert in live data (MissingLevel, Stale) proven on a synthetic scratch test. Still open for the team: which-Kyle on 4 projects; real per-level/archetype hour numbers; NetSuite go-live; confirm the "missing level"/"stale" definitions match intent. **Next: Step 6 (hardening & polish, Sonnet).** The generator is still authoritative (only encodable human input so far); it becomes bootstrap-only once real actuals/dates/probabilities are pasted in at scale — see rule 2.
