...
**Current status: Excel roadmap COMPLETE — Steps 1–7 delivered & verified** (`workbook/US_MA_Tax_Scheduling_Tracker_v2_2026-07-19.xlsx`, 12 tabs). Steps 2–6: applied the team's 7 Review decisions (Patch MIF_1 → Active); **Templates** (archetypes → hrs/wk by level + phase shape) + time-phased Capacity; **Actuals** + **Variance** (NetSuite import, matching, estimate-vs-actual calibration); **Check-in** (weekly agenda); protection + **data-health panel** + print setup. Step 7 added **Scenario** — what-if overrides (violet Deals columns AA–AC) driving a read-only Scenario tab (per-person Live-vs-Scenario impact + heatmap), strictly isolated: with no overrides it equals Live cell-for-cell and never touches Capacity/Check-in (verified both directions; a 0% scenario prob on B2V_1 dropped only the scenario). Verified: 0 errors across 49,337 cells; Steps 1–6 numbers unchanged. **Open for the team (not code):** which-Kyle on 4 projects; real per-level/archetype hour numbers; NetSuite go-live; confirm "missing level"/"stale" definitions. **No further steps** — remaining work is the team's data tuning + ops, or reviving the web app (PLAN §10). The generator is still authoritative (only encodable human input so far); bootstrap-only once real actuals/dates/probabilities are entered at scale — see rule 2.
