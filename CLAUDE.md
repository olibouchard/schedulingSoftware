...
**Current status: Steps 1–10 delivered & verified** (`workbook/US_MA_Tax_Scheduling_Tracker_v2_2026-07-19.xlsx`, 12 tabs; 0 errors across 58,481 cells at last full verify). Feedback Round 1 (PLAN.md §5A) decisions recorded July 21. **Step 10 done — timeline flags:** renamed `Deals!I/J` headers to **Kick-off date / Delivery date** (cells/formats/formulas all unchanged — zero churn) and added `Deals!BQ` **Timeline status** (visible calc), 7 branches off `Settings!B19`: Tentative (a date blank) → Not started (>7 days out) → Kick-off ≤1 wk → In flight → Delivery ≤2 wks → Overdue (past delivery, still Active) → Delivered (past delivery, any other lifecycle). No volatile `TODAY()`. Surfaced on **Check-in §2** as a new column (Overdue rows highlight red). **Invariant verified:** every real deal has a blank date so all show Tentative — no capacity/fee number moved; a synthetic date-matrix scratch test drove all 7 branches + an as-of +7-day bump against an independent Python transcription of the spec. **Prior engine still placeholder-gated:** Step 9's fee→hours needs the team's rate card + split-% matrix (Templates) to compute real hours. **Next: Step 11 (Dashboard + tab diet, Opus)** — the D/MD share view + hide all but Dashboard·Deals·Assignments (Step 11 also adds the deferred visible unstaffed-funded-level flag). Older open items: which-Kyle on 4 projects; NetSuite go-live; "missing level"/"stale" definitions.
